...
import { ReturnRequest, InvestigationCase } from '@/../types/database';
...
type TabType = 'incoming' | 'confirmed' | 'disputes' | 'cases' | 'qr' | 'reports' | 'settings';
...
export default function DistributorDashboard() {
  ...
  useEffect(() => {
    if (tabParam && ['incoming', 'confirmed', 'disputes', 'cases', 'qr', 'reports', 'settings'].includes(tabParam)) {
  ...
  const fetchReturns = async () => {
    ...
    try {
      ...
      const resCases = await fetch('/api/intelligence/cases?stakeholder=distributor');
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="text-base font-bold text-slate-900">
              Custody & Discrepancy Cases ({cases.length})
            </h2>
            <button
          ...
          {cases.length === 0 ? (
            <div className="p-8 text-center bg-white border border-slate-200 rounded-2xl shadow-sm">
              ...
              <p className="text-sm font-semibold text-slate-700">No active custody cases</p>
              <p className="text-xs text-slate-500 mt-1">
                Quantity discrepancies and custody handoff disputes will be automatically grouped here.
              </p>
            </div>
          ) : (
